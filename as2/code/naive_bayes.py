...
class NaiveBayes:
    ...
    def fit(self, X, y):
        ...
        for d in range(D):
            for c in range(C):
                # calculate conditioanl probability for every classes and for every feature;
                class_num=np.sum(X[y==c,d])+beta
                p_xy[d,c]=class_num/counts[c]
